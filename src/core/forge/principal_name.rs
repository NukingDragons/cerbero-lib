...
pub fn new_principal_or_srv_inst(name: &str, realm: &str) -> PrincipalName
{
	...
	return PrincipalName { name_type: name_type, name_string: name_parts };
}
...
{
	return new_principal_name(&format!("{}@{}", &user.name, &user.realm), principal_names::NT_ENTERPRISE);
}

pub fn new_principal_name(name: &str, name_type: i32) -> PrincipalName
{
	return PrincipalName { name_type: name_type, name_string: spn_to_service_parts(name) };
}
