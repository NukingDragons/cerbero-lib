...
/// from user data
pub fn build_as_req(user: KrbUser, cipher: Option<&Cipher>, etypes: Option<Vec<i32>>) -> AsReq
{
	let mut as_req_builder = KdcReqBuilder::new(user.realm).username(user.name).request_pac();
...
pub fn build_tgs_req(user: KrbUser,
                     server_realm: String,
                     tgt: Ticket,
                     cipher: &Cipher,
                     s4u2options: S4u,
                     ...
                     -> TgsReq
{
	...
	match s4u2options
	{
		...
		S4u::S4u2self(impersonate_user, user_service) =>
		{
			...
			tgs_req_builder =
				tgs_req_builder.push_padata(new_pa_data_pa_for_user(impersonate_user, cipher)).sname(Some(service));
		},
