...
pub fn as_rep_to_crack_string(username: &str, as_rep: &AsRep, crack_format: CrackFormat) -> String
{
	let etype = as_rep.enc_part.etype;
	let realm = &as_rep.crealm;

	let (salt, ciphertext) = divide_salt_and_ciphertext(etype, as_rep.enc_part.cipher.to_vec());
	let salt_hexa = arr_u8_to_hexa_string(&salt);
	let cipher_hexa = arr_u8_to_hexa_string(&ciphertext);

	match crack_format
	{
		CrackFormat::Hashcat => format!("$krb5asrep${}${}@{}:{}${}", etype, username, realm, salt_hexa, cipher_hexa),
		CrackFormat::John => format!("$krb5asrep${}@{}:{}${}", username, realm, salt_hexa, cipher_hexa),
	}
}
...
pub fn tgs_to_crack_string(username: &str, service: &str, ticket: &Ticket, crack_format: CrackFormat) -> String
{
	...
	match crack_format
	{
		CrackFormat::Hashcat => match etype
		{
			...
			{
				format!("$krb5tgs${}${}${}$*{}*${}${}", etype, user, realm, serv, salt_hexa, cipher_hexa)
			},
			_ => format!("$krb5tgs${}$*{}${}${}*${}${}", etype, user, realm, serv, salt_hexa, cipher_hexa),
		},
		CrackFormat::John => format!("$krb5tgs${}@{}${}:{}${}", user, realm, serv, salt_hexa, cipher_hexa),
	}
}

fn divide_salt_and_ciphertext(etype: i32, cipher: Vec<u8>) -> (Vec<u8>, Vec<u8>)
{
	let mut salt;
	let mut ciphertext;
